...
const RoomCard: React.FC<RoomCardProps> = ({ id, name, members, options = { showHeader: true }, index }) => {
    ...
    <View style={[globalStyles.card, { backgroundColor: surfaceColor }]}>
      {options?.showHeader ? (
        <View style={roomStyles.header}>
          ...
          <TouchableOpacity style={{ elevation: 8 }} onPress={() => navigation.navigate("RoomDetails", { index, room: { id, name, members } })}>
